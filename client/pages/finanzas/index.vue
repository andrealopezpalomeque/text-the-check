    ...
    <div v-else class="flex flex-col gap-4">
      ...
      <div class="md:hidden px-3 space-y-4">
        <div
          v-for="payment in recurrents"
          :key="payment.id"
          class="bg-ttc-surface rounded-xl border border-ttc-border shadow-sm overflow-hidden"
        >
          <!-- Payment Header -->
...
async function togglePaymentStatus(recurrentId, month) {
  ...
  togglingPayment.value = null;
  if (result) {
    useFinanzasToast("success", `Pago marcado como ${!currentStatus ? "pagado" : "no pagado"}`);
    // Preserve current sort order after status change
    if (currentSortOrder.value.name) {
      applySortOrder(currentSortOrder.value);
    }
  } else {
    useFinanzasToast("error", recurrentStore.error || "Error al actualizar el estado del pago");
  }
}
...
async function createPaymentForMonth(recurrentId, month, isPaid = false, year = null) {
  ...
  if (result) {
    useFinanzasToast("success", "Instancia de pago creada");
    // Preserve current sort order after creating payment
    if (currentSortOrder.value.name) {
      applySortOrder(currentSortOrder.value);
    }
  } else {
    useFinanzasToast("error", recurrentStore.error || "Error al crear instancia de pago");
  }
}
...
// ----- Watchers ---------
// FAB trigger: open new recurrent modal when FAB is pressed on this page
watch(isNewPaymentModalOpen, (open) => {
  if (open && newPaymentModalType.value === 'recurrent') {
    newPaymentModal.value?.showModal()
