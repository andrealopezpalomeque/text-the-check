    ...
    <div class="flex flex-col gap-4 px-3">
      ...
      <div v-else class="flex flex-col gap-3">
        <div
          ...
          :key="category.id"
          class="bg-ttc-surface rounded-xl border border-ttc-border shadow-sm p-4 flex items-center justify-between gap-4"
        >
...
async function addCategory() {
  ...
  if (result) {
    useFinanzasToast("success", "Categoría creada correctamente");
    // Reset form
    newCategory.value = {
      name: '',
      color: '#808080'
    ...
    useFinanzasToast("error", categoryStore.error || "Error al crear la categoría");
  }
}
...
async function saveEditCategory() {
  ...
  if (result) {
    useFinanzasToast("success", "Categoría actualizada correctamente");
    editModal.value.closeModal();
  } else {
    useFinanzasToast("error", categoryStore.error || "Error al actualizar la categoría");
  }
}
...
async function confirmDelete() {
  ...
  if (result) {
    useFinanzasToast("success", "Categoría eliminada correctamente");
  } else {
    useFinanzasToast("error", categoryStore.error || "Error al eliminar la categoría");
  }
