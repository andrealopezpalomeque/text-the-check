          ...
          <div class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 gap-4 pt-4">
              ...
              <div>
                ...
                <!-- View mode -->
                <div v-if="!isEditingAliases" class="flex flex-wrap gap-2 mt-1">
                  <span
...
const startEditingAliases = () => {
  editAliases.value = [...(firestoreUser.value?.aliases || [])]
  newAliasInput.value = ''
...
const addAlias = () => {
  const alias = newAliasInput.value.trim().toLowerCase().replace(/^@/, '')
  if (!alias) return
  if (editAliases.value.includes(alias)) {
    aliasError.value = 'Ese alias ya existe'
    return
  }
  aliasError.value = null
...
const saveAliases = async () => {
  ...
  try {
    await userStore.updateUserAliases(firestoreUser.value.id, editAliases.value)
    // Update auth state so view mode reflects the change immediately
    // (firestoreUser from useAuth is readonly, so access the underlying useState)
    const fsUser = useState('firestore-user')
    fsUser.value = { ...fsUser.value, aliases: [...editAliases.value] }
    isEditingAliases.value = false
  } catch (error) {
    console.error('Error saving aliases:', error)
    aliasError.value = 'Error al guardar. Intenta nuevamente.'
  } finally {
