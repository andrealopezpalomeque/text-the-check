      ...
      <div class="bg-ttc-card border border-ttc-border rounded-xl p-8">
        ...
        <div v-if="loginStep === 'google-prompt'">
          <p class="font-body text-sm text-ttc-text mb-4">
            ¿Ya tenés una cuenta creada por WhatsApp?
          </p>
        ...
        <div v-if="loginStep === 'merge-phone'">
          <p class="font-body text-sm text-ttc-text-muted mb-4">
            Ingresá tu número de WhatsApp para vincular tu cuenta de Google
          </p>
          <label class="block font-body text-sm font-medium text-ttc-text mb-2">
            Tu número de WhatsApp
          </label>
...
const verifyMergeOTP = async () => {
  ...
  try {
    ...
    if (!email) {
      errorMsg.value = 'Error: no se encontró el email de Google'
      return
    }
