...
const recurrentStore = useFinanzasRecurrentStore();
const paymentStore = useFinanzasPaymentStore();
const templateStore = useFinanzasTemplateStore();
...
async function fetchPaymentDetails(paymentId) {
  ...
  try {
    ...
    useFinanzasToast("error", "Error al cargar los detalles del pago");
  } finally {
...
async function savePayment() {
  ...
  if (!user) {
    useFinanzasToast("error", "Debés iniciar sesión para guardar pagos");
    return;
  }
  ...
  try {
    ...
    if (props.isRecurrent && !props.isEdit) {
      useFinanzasToast("error", "Los pagos fijos no se pueden crear desde este formulario");
    ...
    if (props.isEdit && props.paymentId) {
      ...
      if (result) {
        useFinanzasToast("success", props.isReview ? "Pago revisado correctamente" : "Pago actualizado correctamente");
        emit("onCreated");
        closeModal();
      } else {
        useFinanzasToast("error", paymentStore.error || "Error al actualizar el pago");
      }
    } else {
      // Create new payment
      ...
      if (result && result.success) {
        useFinanzasToast("success", "Pago creado correctamente");
        emit("onCreated");
        ...
        useFinanzasToast("error", paymentStore.error || "Error al crear el pago");
      }
    }
  } catch (error) {
...
async function deletePayment() {
  ...
  try {
    ...
    if (result) {
      useFinanzasToast("success", "Pago eliminado correctamente");
      emit("onCreated");
      closeModal();
    } else {
      useFinanzasToast("error", paymentStore.error || "Error al eliminar el pago");
    }
  } catch (error) {
