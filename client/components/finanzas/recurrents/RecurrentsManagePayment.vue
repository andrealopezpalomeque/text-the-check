...
async function savePayment() {
  ...
  try {
    ...
    if (props.isEdit && props.paymentId) {
      ...
      if (result) {
        useFinanzasToast("success", "Pago actualizado correctamente");
        closeModal();
      } else {
        useFinanzasToast("error", recurrentStore.error || "Error al actualizar el pago");
      }
    } else {
      // Create new payment
      ...
      if (result) {
        ...
        useFinanzasToast("error", recurrentStore.error || "Error al crear el pago");
      }
    }
  } catch (error) {
